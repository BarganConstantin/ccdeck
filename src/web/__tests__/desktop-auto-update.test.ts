...
// installs itself once the app has been quiet for a while. Quiet is about the
// PERSON, not about the agents: the owner decided on 2026-09-22 that a restart
// under a running agent is fine, because the hook POSTs, exits 0 and never
// speaks to the agent about it, so the cost is a gap in the drawing of a turn
// and the alternative is an app that stays versions behind.
...
describe("installing an update by itself", () => {
  ...
  it("does not wait for the agents: a running or waiting session is not its business", () => {
    // Deliberate, and the reason is in auto-update.mjs. A deck that goes away
    // for a second costs a hook one POST it does not report, and waiting for
    // every agent on a busy machine to be idle is how an app never updates.
    expect(canInstallQuietly({ ...ripe, running: 3, waiting: 2 } as never)).toBe(true);
  });
...
describe("the quiet the rule measures", () => {
  ...
  it("is lost the moment somebody is there, so an app being clicked through does not update mid-click", () => {
...
describe("the app's wiring", () => {
  it("measures the quiet on the same tick that draws the tray", () => {
    ...
    // What the agents are doing is not asked, on purpose.
    expect(fn).not.toMatch(/snapshot\.(waiting|running)/);
