...
describe("selecting an agent opens its details (#814)", () => {
  it("opens the detail panel on a plain selection, and not on Shift+click", () => {
    expect(selectAgent).toMatch(/if \(!additive\) setDetailOpen\(true\);/);
  });
  ...
  it("is reached by every way a single agent gets selected", () => {
    ...
    expect(appCode).toMatch(/onNodeClick=\{\(e, n\) => \{[\s\S]*?selectAgent\(n\.id, e\.shiftKey\);/);
  ...
  it("makes the tour's inspect step true (#817)", () => {
    expect(WELCOME_STEPS.some(s => /^Click any node: its prompt, every tool call/.test(s.line))).toBe(true);
  });
});
