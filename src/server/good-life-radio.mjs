const GOOD_LIFE_URL = "https://www.youtube.com/@TheGoodLiferadio";
...
export async function fetchGoodLifeRadio({ fetchImpl } = {}) {
  const now = Date.now();
  ...
  inflight = (async () => {
    try {
      const response = await get(GOOD_LIFE_URL, {
      ...
      return settle(video ? { id: "good-life-radio", label: "The Good Life Radio — Live", video } : null);
    } catch {
      return settle(null);
    } finally {
      inflight = null;
...
export function forgetGoodLifeRadio() {
  cache = null;
  cacheAt = 0;
  cacheTtl = 0;
  inflight = null;
