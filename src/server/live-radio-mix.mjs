const RADIO_MIX_URL = "https://www.youtube.com/@liveradiomix";
...
function currentLiveVideo(html) {
  const live = html.indexOf('"liveBadgeText":"LIVE"');
  if (live < 0) return null;
  const window = html.slice(Math.max(0, live - 2_000), live + 2_000);
  return window.match(/"videoId":"([\w-]{11})"/)?.[1] ?? null;
}
...
export async function fetchLiveRadioMix({ fetchImpl } = {}) {
  ...
  inflight = (async () => {
    try {
      const response = await get(RADIO_MIX_URL, {
      ...
      return settle(video ? { id: "radio-mix", label: "Radio Mix — Live", video } : null);
    } catch {
      return settle(null);
    } finally {
      inflight = null;
...
export function forgetLiveRadioMix() {
  cache = null;
  cacheAt = 0;
  cacheTtl = 0;
  inflight = null;
