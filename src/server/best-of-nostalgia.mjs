const NOSTALGIA_URL = "https://www.youtube.com/@bestofnostalgia";
...
export async function fetchBestOfNostalgia({ fetchImpl } = {}) {
  ...
  inflight = (async () => {
    try {
      const response = await get(NOSTALGIA_URL, {
      ...
      return settle(video ? { id: "best-of-nostalgia", label: "Best of Nostalgia — Live", video } : null);
    } catch {
      return settle(null);
    } finally {
      inflight = null;
...
export function forgetBestOfNostalgia() {
  cache = null;
  cacheAt = 0;
  cacheTtl = 0;
  inflight = null;
